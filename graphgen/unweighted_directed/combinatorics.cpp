...
double binomial(int n, int x, double p) {		//	returns the binomial distribution, n trials, x successes, p probability
	...
	double log_b=0;
	log_b+=log_combination(n, x)+x*std::log(p)+(n-x)*std::log(1-p);
...
double poisson (int x, double mu) {
	...
	return (std::exp(-mu+x*std::log(mu)- log_factorial(x)));
...
int shuffle_s(std::deque<int> &sq) {
...
double compute_self_links(int k, int n, int x) {
	...
	double prod= log_combination(n/2, k-x) + log_combination(k-x, x) + (k-2*x) * std::log(2) - log_combination(n, k);
