...
int print_network(deque<set<int> > & Ein, deque<set<int> > & Eout, 
...
int benchmark(bool excess, bool defect, int num_nodes, double  average_k, 
    ...
    bool  fixed_range, deque<set<int> > &output_Eout, 
  ...
  if(internal_degree_and_membership(mixing_parameter, overlapping_nodes, 
      overlap_membership, num_nodes, member_matrix, excess, defect, 
      degree_seq_in, degree_seq_out, num_seq, internal_degree_seq_in, 
      internal_degree_seq_out, fixed_range, nmin, nmax, tau2)==-1)
    return -1;
  ...
  if(build_subgraphs(Ein, Eout, member_matrix, member_list, link_list_in, 
      link_list_out, internal_degree_seq_in, degree_seq_in, 
      internal_degree_seq_out, degree_seq_out, excess, defect)==-1)
    return -1;  
...
//  srand_seed(seed);
  ...
  benchmark(p.excess, p.defect, p.num_nodes, p.average_k, p.max_degree,
            ...
            output_Eout, output_member_list);
}