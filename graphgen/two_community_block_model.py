...
def unweighted_two_community_graph(random_state, N, mu, avg_degree):
    ...
    nodes = set(range(N))
    community1 = set(random_state.random_sample(nodes, N/2))
    ...
    # Generate edges
    num_edges_com1tocom2, num_edges_com2tocom1 = random_state.poisson(B * (N/2.0)**2, size=2)
    num_edges_com1tocom1, num_edges_com2tocom2 = random_state.poisson(A * (N/2.0)**2, size=2)
...
def add_edges_to_graph(graph, com1_set, com2_set, num_edges_to_add, random_state):
    com1_set = list(com1_set)
    com2_set = list(com2_set)
    for i in range(num_edges_to_add):
        tail = random_state.choice(com1_set)
        head = random_state.choice(com2_set)
        tries = 0
...
def uniform_weighted_two_community_graph(random_state, N, mu, avg_degree, lower_bound=0,
                                         upper_bound=1):

    graph = unweighted_two_community_graph(random_state, N, mu, avg_degree)
    weights = random_state.uniform(lower_bound, upper_bound, size=nx.number_of_edges(graph))
...
def gamma_weighted_two_community_graph(random_state, N, mu, avg_degree, EE_W=30000.0,
                                       negative_weights=False):

    graph = unweighted_two_community_graph(random_state, N, mu, avg_degree)

    weights = random_gamma(random_state, EE_W, nx.number_of_edges(graph))
...
def random_gamma(random_state, mean, size=1, shape=4.0):
    ...
    if mean > 0:
        ...
        shape = shape
        scale = mean / shape
        return -1. * random_state.gamma(shape, size=size, loc=0.0, scale=scale)
