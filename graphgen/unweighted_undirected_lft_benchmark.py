...
def generate_graph(network_params, command_file, path):
    ...
    # Run program
    subprocess.call([command_file, "-f", network_params['param_file']])
...
def read_LFR_output(edge_file, community_file):
    ...
    col1 = np.array(list(col1))
    col2 = np.array(list(col2))
    edge_list = list(zip(col1, col2))
    
    nodes, clusters = utilities.readcol(community_file, 'ii',delimiter='\t')
    nodes = np.array(list(nodes))
    community_list = list(zip(nodes, list(clusters)))
